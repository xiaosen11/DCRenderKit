...
| 实现 | 下采样 | 上采样 | 来源 |
|---|---|---|---|
| **DCR (本实现)** | 2×2 box (4-tap, 均权 1/4) | 9-tap tent (1-2-1/2-4-2/1-2-1 / 16) | 独立 Swift/Metal 实现 |
...
- 来源: empirical hand-tune，无独立 fit 过程
...
**依据**: Dual Kawase / box-pyramid bloom 特性 —— 每级 2× 下采样 + upsample tent 扩散 radius 呈指数（金字塔深度越深扩散越远）。具体扩散距离由 depth anchor `log2(shortSide / 135)` 决定（135 是"1080 ÷ 2³"的反推，一个 1080p 下 3 级 pyramid 的历史选择 —— 在 64×64 下 `max(3, ⌊log₂(64/135)⌋) = 3` 级，pyramid 8×8 为最深）。

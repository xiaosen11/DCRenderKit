...
|---|---|---|
| **DCR HS (本实现)** | Fast Guided Filter (He & Sun 2015) | 独立 Swift/Metal 实现 |
...
**Guided filter radius `p = 0.012` (fraction of quarter-res short side) (B.4 findings)**:
- 1080p → quarter-res 270 px → 3.2 px quarter-res radius → **~13 px full-res radius → 1.2% 短边**
- 4K → ~26 px full-res → 1.2% 同
- 8K → ~52 px full-res → 1.2% 同
- **本值 = 分辨率自适应的 fixed-percentage radius**
- **硬依据**: TMO (tone mapping operator) 文献的 edge-preserving base extractor 典型 σ / radius **1-5% of image dimension** ([Eilertsen 2017 TMO survey](https://www.cl.cam.ac.uk/~rkm38/pdfs/eilertsen2017tmo_star.pdf) §4). DCR HS 1.2% 落在低端但**在范围内**
- **为什么比 Clarity (1.9%) 更小**: HS 是 tone correction use case —— base 应该是 "neighborhood 的 general brightness"，不是 "whole region 的平均"。更紧的 radius 服务此意图。对照 Clarity 需要 "broader 的 base 让 detail residual 抓中频纹理"，所以用更大 radius

**Smoothstep 窗口 `[0.25, 0.85]` / `[0.15, 0.75]` (B.4 发现，硬依据)**:
- **全部四个端点与 Norman Koren simplified Zone System midpoints 精确匹配** ([Koren Zone System](https://www.normankoren.com/zonesystem.html) 8-bit midpoints: 13, 38, **64**, 90, 115, 141, 166, **192**, **218**, 243)
  - Highlight 下端 `0.25` ↔ Zone III midpoint `64/255 = 0.251` (差 <0.005)
  - Highlight 上端 `0.85` ↔ Zone IX midpoint `218/255 = 0.855` (差 <0.005)
  - Shadow 下端 `0.15` ↔ Zone II midpoint `38/255 = 0.149` (差 <0.005)
  - Shadow 上端 `0.75` ↔ Zone VIII midpoint `192/255 = 0.753` (差 <0.005)
- **语义**:
  - Highlight smoothstep 从 Zone III 过渡到 Zone IX；eased smoothstep 把**强响应区**集中在中点 Zone VI (`141/255 = 0.553`) 以上 —— "真高光"
  - Shadow smoothstep 从 Zone II 过渡到 Zone VIII；强响应区集中在中点 Zone V (`115/255 = 0.451`) 以下 —— "真阴影"
  - 窗口覆盖 6 个 zones （占 gamma 空间 60%）给出平滑过渡，不是硬截断
- **硬依据**: Zone System 是 Ansel Adams 1981 标准化的 tonal 框架；用 Zone midpoints 做 smoothstep 窗口端点 = 把 "filter 定义哪些像素是高光/阴影" 锚在公认 tonal 框架上，不是 arbitrary pick
- **重要说明**: 这些常数**早期写入 shader 时未标注为 Zone 中点**，曾被视为 empirical magic number。Session B B.4 发现 4 个端点全部与 Norman Koren Zone midpoints 精确匹配到 <0.005，超越巧合范围 —— **追认**为 Zone System derivation
...
- 无已发表论文给出"高光补偿 × 0.35, 阴影补偿 × 0.50"的理论支持 —— 这些是 empirical aesthetic choice
